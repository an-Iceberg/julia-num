...
# ∂¹
# Todo: theoretically these should work but test that it really does

function ∂1_2(f, x⃗, i, h)
  h⃗ = zeros(length(x⃗))
  h⃗[i] = h
  return d1_2(f, x⃗, h⃗)
end

function ∂1_4(f, x⃗, i, h)
  h⃗ = zeros(length(x⃗))
  h⃗[i] = h
  return d1_4(f, x⃗, h⃗)
end

function ∂1_6(f, x⃗, i, h)
  h⃗ = zeros(length(x⃗))
  h⃗[i] = h
  return d1_6(f, x⃗, h⃗)
end

function ∂1_8(f, x⃗, i, h)
  h⃗ = zeros(length(x⃗))
  h⃗[i] = h
  return d1_8(f, x⃗, h⃗)
end
...
function ∂2_2(f, x⃗, i, h)
  h⃗ = zeros(length(x⃗))
  h⃗[i] = h
  return d2_2(f, x⃗, h⃗)
end

function ∂2_4(f, x⃗, i, h)
  h⃗ = zeros(length(x⃗))
  h⃗[i] = h
  return d2_4(f, x⃗, h⃗)
end

function ∂2_6(f, x⃗, i, h)
  h⃗ = zeros(length(x⃗))
  h⃗[i] = h
  return d2_6(f, x⃗, h⃗)
end

function ∂2_8(f, x⃗, i, h)
  h⃗ = zeros(length(x⃗))
  h⃗[i] = h
  return d2_8(f, x⃗, h⃗)
end
...
function ∂3_2(f, x⃗, i, h)
  h⃗ = zeros(length(x⃗))
  h⃗[i] = h
  return d3_2(f, x⃗, h⃗)
end

function ∂3_4(f, x⃗, i, h)
  h⃗ = zeros(length(x⃗))
  h⃗[i] = h
  return d3_4(f, x⃗, h⃗)
end

function ∂3_6(f, x⃗, i, h)
  h⃗ = zeros(length(x⃗))
  h⃗[i] = h
  return d3_6(f, x⃗, h⃗)
end
...
function ∂4_2(f, x⃗, i, h)
  h⃗ = zeros(length(x⃗))
  h⃗[i] = h
  return d4_2(f, x⃗, h⃗)
end

function ∂4_4(f, x⃗, i, h)
  h⃗ = zeros(length(x⃗))
  h⃗[i] = h
  return d4_4(f, x⃗, h⃗)
end

function ∂4_6(f, x⃗, i, h)
  h⃗ = zeros(length(x⃗))
  h⃗[i] = h
  return d4_6(f, x⃗, h⃗)
end
...
function ∂5_2(f, x⃗, i, h)
  h⃗ = zeros(length(x⃗))
  h⃗[i] = h
  return d5_2(f, x⃗, h⃗)
end

function ∂5_4(f, x⃗, i, h)
  h⃗ = zeros(length(x⃗))
  h⃗[i] = h
  return d5_4(f, x⃗, h⃗)
end

function ∂5_6(f, x⃗, i, h)
  h⃗ = zeros(length(x⃗))
  h⃗[i] = h
  return d5_6(f, x⃗, h⃗)
end
...
function ∂6_2(f, x⃗, i, h)
  h⃗ = zeros(length(x⃗))
  h⃗[i] = h
  return d6_2(f, x⃗, h⃗)
end

function ∂6_4(f, x⃗, i, h)
  h⃗ = zeros(length(x⃗))
  h⃗[i] = h
  return d6_4(f, x⃗, h⃗)
end

function ∂6_6(f, x⃗, i, h)
  h⃗ = zeros(length(x⃗))
  h⃗[i] = h
  return d6_6(f, x⃗, h⃗)
end
...
# Todo: test if this works. Theoretically it should
function ∇1_2(f, x⃗, h)
  return d1_2(f, x⃗, h)
  # return [∂1_2(f, x⃗, i, h) for i in 1:length(x⃗)]
end
